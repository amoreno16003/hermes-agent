...
def build_parser(
    ...
    p_sync = sub.add_parser(
        "sync-discord",
        help="Create missing Discord channels for projects (idempotent)",
    )
    p_sync.add_argument(
        "--include-archived",
        action="store_true",
        help="Also provision channels for archived projects",
    )
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    # Mirror the new project as a Discord channel when the feature is on.
    # Best-effort by contract: a Discord outage or a missing permission must
    # never fail `hermes project create`.
    try:
        ...
        if _pc.is_enabled():
            ...
            if cid:
                ...
                print(
                    "Discord: channel provisioning skipped (see logs); "
                    "retry with `hermes project sync-discord`."
                )
            proj = None
            with pdb.connect_closing() as conn:
                proj = pdb.get_project(conn, pid)
